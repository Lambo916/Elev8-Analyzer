...
(() => {
  ...
  async function drawFooter(doc, pageNum, totalPages, iconDataUrl) {
    ...
    // Left side: CompliPilot logo + text (square logo)
    const footerLogoSize = 5;  // mm - square logo in footer
    
    if (iconDataUrl) {
      try {
        doc.addImage(iconDataUrl, "PNG", CONTENT.left, mainFooterY - 4, footerLogoSize, footerLogoSize, "", "FAST");
      } catch (e) {
        // Silent fail for footer logo
      }
    }
    
    doc.text("Generated by CompliPilot · YourBizGuru.com", CONTENT.left + footerLogoSize + 2, mainFooterY);
