...
(() => {
  ...
  function drawHeader(doc, pageNum, iconDataUrl) {
    ...
    // CompliPilot logo with proper aspect ratio
    const logoWidth = 20;  // mm - wider for rectangular logo
    const logoHeight = 12; // mm - maintains aspect ratio
    ...
    if (iconDataUrl) {
      try {
        ...
        // Draw logo without circular ring
        doc.addImage(iconDataUrl, "PNG", logoX, logoY, logoWidth, logoHeight, "", "FAST");
      } catch (e) {
    ...
    doc.text(getToolkitName(), x + logoWidth + 4, y + 7);
    ...
    doc.text(dateStr, x + logoWidth + 4, y + 12);
  ...
  // ---- Footer Drawing ------------------------------------------------------
  async function drawFooter(doc, pageNum, totalPages, iconDataUrl) {
    ...
    // Left side: CompliPilot logo + text
    const footerLogoWidth = 8;
    const footerLogoHeight = 5;
    
    if (iconDataUrl) {
      try {
        doc.addImage(iconDataUrl, "PNG", CONTENT.left, mainFooterY - 4, footerLogoWidth, footerLogoHeight, "", "FAST");
      } catch (e) {
        // Silent fail for footer logo
      }
    }
    
    doc.text("Generated by CompliPilot · YourBizGuru.com", CONTENT.left + footerLogoWidth + 2, mainFooterY);
  ...
  class PDFWriter {
    ...
    finalize() {
      // Draw footers on all pages with CompliPilot logo
      for (let i = 1; i <= this.totalPages; i++) {
        this.doc.setPage(i);
        drawFooter(this.doc, i, this.totalPages, this.iconDataUrl);
      }
    }
  }
